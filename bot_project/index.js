...
async function startNazeBot() {
	...
	setTimeout(() => {
		global.intervalSholat = setInterval(async() => {
			...
			if (detik !== '00') return;
			// ✅ Kumpulkan dulu semua target grup
            const targetGroups = Object.entries(global.db.groups)
              .filter(([_, settings]) => settings.waktusholat);
            
            // ✅ Kirim semua sekaligus — tidak ada yang menunggu
            await Promise.allSettled(
              targetGroups.map(([idnya]) =>
                naze.sendMessage(idnya, {
                  text: `Waktu *${sholat}* telah tiba, ambilah air wudhu dan segeralah shalat🙂.\n\n` +
                        `*${waktu.slice(0, 5)}*\n_untuk wilayah ${global.timezone} dan sekitarnya._`
                }, {
                  ephemeralExpiration: store?.messages[idnya]?.array?.slice(-1)[0]
